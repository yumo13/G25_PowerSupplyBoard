...
void CanController::initialize(CAN_HandleTypeDef *hcan)
{
    getInstance().initInternal(hcan);
}
...
// メッセージハンドラ
void CanController::onReceive(uint32_t id, CanMessageHandler handler)
{
    getInstance().messageHandlers_[id] = handler;
}

void CanController::onReceiveAny(CanMessageHandler handler)
{
    getInstance().globalHandler_ = handler;
}

void CanController::onError(CanErrorHandler handler)
{
    getInstance().errorHandler_ = handler;
}
...
// 割り込み処理
void CanController::handleRxInterrupt(uint32_t fifo)
{
    processRxFifo(fifo);
}
